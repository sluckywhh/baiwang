...
var DWZ = {
	...
	ajaxDone:function(json){
		if(json.statusCode == DWZ.statusCode.error) {
			if(json.message && alertMsg) alertMsg.error(json.message,{javaException:json.javaException});
...
(function($){
	...
	$.fn.extend({
		...
		ajaxUrl: function(op){
			var $this = $(this);
			var update=$this.attr("id")!="ajaxHidden";
			//只加载ajax数据时不更新页面
			if(update){
				$this.trigger(DWZ.eventType.pageClear);
			}
			op.data = op.data||{};
			//IE浏览器在url长度大于2048时ajax会报错
			//if($.browser.msie&&$.browser.version=="6.0"&&typeof op.data=="string"&&op.data.length>800){
			//	op.type = "POST";
			//}
			$.ajax({
				type: op.type || 'POST',
				...
				data: op.data,
				cache: false,
				async: op.data.async==false?false:true,
				global: op.data.global==false?false:true,
				success: function(response){
					var json = DWZ.jsonEval(response);
					
					if (json.statusCode==DWZ.statusCode.error){
						if (json.message) alertMsg.error(json.message,{javaException:json.javaException});
					} else {
						if(update){
		...
		inputAlert: function(){
			return this.each(function(){
				...
				$this.focus(function(){
					...
					if (!$(this).val()) getAltBox().show().css("opacity",0.6);
				}).keydown(function(){
					getAltBox().hide();
				});
			});
