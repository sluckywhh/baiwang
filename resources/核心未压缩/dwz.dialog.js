...
(function($){
	$.pdialog = {
		_op:{height:300, width:580, minH:40, minW:50, total:20, max:false, mask:true, resizable:true, drawable:true, maxable:true,minable:true,fresh:true},
		_current:null,
		_zIndex:42,
		getCurrent:function(){
		...
		open:function(url, dlgid, title, options) {
			...
			if(dialog) {
				...
				dialog.find(".dialogHeader").find("h1").html(title).click(function(){
...
//					$.pdialog.switchDialog(dialog);
                ...
                if(op.mask&&$.pdialog._current&&$.pdialog._current.is(":visible")){
                    $.pdialog._current.css("z-index","20");
					dialog.data("prev",$.pdialog._current);
                }
				...
				if(url.indexOf("#")<0) {
					...
					jDContent.loadUrl(url, {}, function () {
						...
						$("form.required-validate", dialog).keydown(function (event) {
		...
		close:function(dialog) {
			...
			if(dialog.data("mask")){
				if(dialog.data("prev")){
					var prev = dialog.data("prev").css("z-index","999");
					$.pdialog.attachShadow(prev);
					$.pdialog._current = prev;
				}else{
