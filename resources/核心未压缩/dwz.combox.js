...
(function($){
	...
	var countStrLength=function (str){
		var _str =str+"";
		if(!_str){
			return 0;
		}
	...
	$.extend($.fn, {
		...
		comboxOption: function(selector, box){
			return this.each(function(){
				$(">a", this).click(function(){
					...
					var text = $this.text();
					var maxL = (selector.width()-10)/12;
					if(countStrLength(text)>maxL){
		...
		combox:function(){
			...
			return this.each(function(i){
				...
				var label = $this.children().eq($this[0].selectedIndex).text();
				...
				var awidth = $this.attr("awidth")||84;
				var maxL = (awidth-10)/12;
				if(countStrLength(label)>maxL){
					label = label.substring(0,maxL)+"...";
				}
				awidth = "style='width:"+awidth+"px'";
				select += '<a href="javascript:" class="'+$this.attr("class")+'" name="' + name +'" value="' + value + '" ' + awidth + '>' + label +'</a></div></div>';
				var options = '<ul class="comboxop" id="op_combox_'+ cid +'">';
				$("option", $this).each(function(){
					var option = $(this);
					options +="<li><a class=\""+ (value==option[0].value?"selected":"") +"\" href=\"#\" value=\"" + option[0].value + "\" title=\""+option[0].title+"\">" + option[0].text + "</a></li>";
				});
				options +="</ul>";
