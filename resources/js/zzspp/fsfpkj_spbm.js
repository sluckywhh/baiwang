var zzspp_fsfpkj_spbm = function(){
	...
	return {
		...
		addRow : function(p) {
			...
			quicklyChoiceSpbm(newLine,hsbz,"007",true,zsfs);
			//切换行
		...
		spxx:function (){
			...
			splxxx.spxx(index,hsbz,"007",true,blxx,"",zsfs);
		},
		//打开费用项目dlg
		...
		addZk:function (){
			...
			$.pdialog.open(ctxPath+"/zzspp_fs_spbm/addZk.do","zzspp_addZk_dlg","添加折扣",{
				width:450,
				height:240,
				callback:function(){
					$("#zkhs").empty();
		...
		openFskjDlg : function(){
			var _this = this;
			$.pdialog.open(ctxPath+"/zzspp_fs_spbm/fskjDlg.do","zzspp_fskjDlg_dlg","销项负数发票代码号码填写、确认",{
				width:480,
				height:245,
				callback:function(){
					$("#dlg_next_bt").click(function(){
						...
						ajaxLoad(ctxPath+"/zzspp_fs_spbm/searchYfp.do",{yfpdm:yfpdm,yfphm:yfphm,tspz:tspz},function(json){
							if(json.errMessage){
		...
		addExtraInfo:function (){
			...
			_$("#zzspp_fsfpkj_spbm_form").append(extraDiv);
		...
		printfp:function (){
			...
			zzspp_fsfpkj_spbm.initPrintData();
			$.cookie("dyfs")=="1"?LODOP.PRINTA():LODOP.PRINT();
		},
		initYLData:function (){
			zzspp_fsfpkj_spbm.initPrintData(true);
		...
		initPage:function(){
			...
			ajaxLoad(ctxPath+"/zzspp_fs_spbm/check.do?tspz="+tspz,{async:false},function(json){
			...
			_$("#kj").click(function(){
				...
				if(valid){
					...
					checkPerm(function(){
						_this.addExtraInfo();
						_$("#zzspp_fsfpkj_spbm_form").submit();
					});
				}
			});
			//差额征税
