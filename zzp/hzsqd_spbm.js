var zzp_hzsqd_spbm = function () {
    // 修改了 js和controller类
    var nowRowCount, szsmStr, sfhs, scsm, isMulti, leng, myse, zsfs, qdbz, ptSzsmStr;// 最后一个字段为清单标志。
    return {
        addRow: function (p) {
            zsfs = _$("#zsfs").val();
            if (nowRowCount == 7) {
            ...
            var tr = _$("<tr index='"
                ...
                + nowRowCount
                + "=''"
                // + " onblur='zzp_hzsqd_spbm.se_onBlur(" + nowRowCount
                // + ");'"
                + " readonly='readonly'"
                + " id='se_"
                + nowRowCount
            ...
            // 绑定事件
            zzp_hzsqd_spbm.addRowDbclick(tr);
        },
        se_onBlur: function (index) {
            ...
            zzp_hzsqd_spbm.reCountHjje();
        },
        addRowDbclick: function (tr) {
            quicklyChoiceSpbm(tr, sfhs, "004", false, zsfs);
            ...
            tr.find("input[id^='spsl'],input[id^='spdj']").bind("change",
                function () {
                    ...
                    zzp_hzsqd_spbm.reCountRow(index, true);
                });
            tr.find("input[id^='je']").bind("change", function () {
                ...
                if (qdbz) {
                    var yje = this.value;// 原金额
                    var je = this.value;
                    _$("#je_1").val((yje * 1).toFixed(2));
                    var se = _$("#se_1").val();
                    if (sfhs) {// 如果含税，转换为不含税的金额。
                        je = je * 1 + Math.abs(se);
                    }
                    if (isMulti == "0") {// 0为单税率
                        zzp_hzsqd_spbm.reCountRow(index);
                        return;
                    } else if (isMulti == "1") {// 如果为多税率
                        if (se != "" && se != null) {
                            var ce = Math.abs(se) - Math.abs(je);
                            if (ce >= 0) {
        ...
        spxx: function () {
            ...
            splxxx.spxx(index, sfhs, "004", true, "", "", zsfs);
        },
        tk: function () {
            _$("#hzdd_spbm0").hide();
            _$("#hzdd_spbm2").hide();
        ...
        xyb: function () {
            ...
            _$("#zsfs").val("0");
            // 申请单
            ajaxLoad(ctxPath + "/hzsqdspbm/sqdjy.do", _$("#hzsqd_spbm_form")
        ...
        toggleCezs: function () {
            ...
            if (cezs.text() == "差额征税") {// 差额征税
                do {
                    zzp_hzsqd_spbm.delRow();
                } while (nowRowCount >= 0);
                nowRowCount = 0;
                zsfs = "2";
                cezs.text("普通征税");
                _$("#zsfs").val("2");
                zzp_hzsqd_spbm.addRow();
        ...
        cxHtSqd: function (args) {
            ...
            for (var i = 1; i <= sqdmx.mxzb.length; i++) {
                zzp_hzsqd_spbm.addRow("not_focus");
        ...
        reDeal: function (args) {
            ...
            // 生成占位行
            for (var index = 1; index <= 8; index++) {
                _$("#fyxmdiv tr:last").prev().after(zzp_hzsqd_spbm
                    .newPlaceHolder());
            }
            // 税种税目
        ...
        hzxz: function () {
            ...
            location.href = ctxPath + "/hzsqdspbm/shxz.do?kpsjq=" + sjq
        ...
        delRow: function () {
            ...
            nowRowCount--;
            _$("#fyxmdiv tr[index]:last").before(zzp_hzsqd_spbm
        ...
        save: function () {
            var boo = zzp_hzsqd_spbm.validateSqd(true);
            if (boo) {
                ajaxLoad(ctxPath + "/hzsqdspbm/save.do",
        ...
        sc: function () {
            ...
            if (boo) {
                ...
                ajaxLoad(ctxPath + "/hzsqdspbm/sc.do",
                    _$("#hzsqd_sc_spbm_form").serialize(), function (json) {
        ...
        dc: function () {
            ...
            location.href = ctxPath + "/hzsqdspbm/dcsqd.do?" + id;
        },
        ...
        dbclickht: function () {
            ...
            var id = sel.attr("rel");
            ajaxLoad(ctxPath + "/hzsqdspbm/drawedit.do?id=" + id, {
                async: false
            }, function (json) {
                _$("#hzdd_spbm0").hide();
                _$("#hzdd_spbm2").hide();
        ...
        printsqd: function () {
            ...
            var xhdwmc = zzp_hzsqd_spbm.considerVal("xhdwmc");
            ...
            LODOP.ADD_PRINT_TEXT(130, 252, 140, 15, zzp_hzsqd_spbm
                .considerVal("xhdwdm"));
            LODOP.SET_PRINT_STYLEA(0, "FontSize", 9);
            LODOP.SET_PRINT_STYLEA(0, "Alignment", 2);
            LODOP.ADD_PRINT_TEXT(130, 602, 140, 15, zzp_hzsqd_spbm
                .considerVal("ghdwdm"));
            LODOP.SET_PRINT_STYLEA(0, "FontSize", 9);
            LODOP.SET_PRINT_STYLEA(0, "Alignment", 2);
            var ghdwmc = zzp_hzsqd_spbm.considerVal("ghdwmc");
...
var wsgp = function () {
    ...
    return {
        math: function () {
            enableButtons(_$("#lg"));
            var zzhm = _$("#zzhm").val() * 1;
            zzhm = zzhmjc;
            var fs = _$("#fs").val().replace(/[^\d.]/g, '');
            if (fs == "") {
                return;
            }
            fs = fs * 1;
            if (fs == 0 || fs == null) {
                alertMsg.error("份数不允许为空或0！");
                disableButtons(_$("#lg"));
                return;
            }
            var qshm = _$("#qshm").val() * 1;
            var jshm = fs + qshm - 1;
            if (jshm > zzhm) {
                alertMsg.error("您所填份数已经超过了库存范围!");
                disableButtons(_$("#lg"));
        ...
        initPage: function () {
            ...
            if (hideMsg) {
                disableButtons(_$("#lg"));
                _$("#fs").attr("disabled", "disabled");
                alertMsg.error(hideMsg);
            }
            limit_money_input(_$("#fs"));
        },
        xz: function (bt) {
            var tds = $(bt).children();
            _$("#fplxdm").val(tds.eq(0).attr("title"));
            _$("#fpdm").val(tds.eq(1).text());
            _$("#qshm").val(tds.eq(2).text());
            _$("#fs").val(tds.eq(4).text());
            var zzhm1 = tds.eq(3).text();
            _$("#zzhm").val(zzhm1);
            zzhmjc = zzhm1;
        },
        lg: function () {
            var fpdm = _$("#fpdm").val();
            var fplxdm = _$("#fplxdm").val();
            if(fpdm=="" || fplxdm==""){
                alertMsg.info("请选择需领购的发票信息");
                return ;
            }
            ajaxLoad(ctxPath + "/wsgp/gp.do", _$("#wsgp_lg").serialize(),
                function (json) {
                    navTabAjaxDone(json);
                })
        }
...
var qycs = function () {
    return {
        xz: function () {
            ajaxLoad(ctxPath + "/qycs/xz.do", _$("#qycs").serialize(),
                function (json) {
                    alertMsg.correct(json.message);
                });
        },
        gx:function () {
            ajaxLoad(ctxPath + "/lxxz/gx.do",{},
                function (json) {
                    alertMsg.correct(json.message);
                });
        }
...
var gfHzxxb_import = function () {
    return {
        checkForm: function () {
            var fpdm = _$("#fpdm").val();
            var fphm = _$("#fphm").val();
            var regexFpdm = /\d{9,12}/;
            var regexFphm = /\d{8}/;
            if (fpdm == "") {
                alertMsg.error("发票代码不能为空！");
                return false;
            } else if (fphm == "") {
                alertMsg.error("发票号码不能为空！");
                return false;
            }
            if (!regexFpdm.test(fpdm) || !regexFphm.test(fphm)) {
                alertMsg.error("填写数据有误，请检查！");
                return false;
            }
