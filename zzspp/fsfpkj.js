var zzspp_fsfpkj = function(){
	...
	return {
		...
		addZk:function (){
			...
			$.pdialog.open(ctxPath+"/zzspp_fs/addZk.do","zzspp_addZk_dlg","添加折扣",{
				width:450,
				height:240,
				callback:function(){
					$("#zkhs").empty();
		...
		openFskjDlg : function(){
			var _this = this;
			$.pdialog.open(ctxPath+"/zzspp_fs/fskjDlg.do","zzspp_fskjDlg_dlg","销项负数发票代码号码填写、确认",{
				width:480,
				height:245,
				callback:function(){
					$("#dlg_next_bt").click(function(){
						...
						ajaxLoad(ctxPath+"/zzspp_fs/searchYfp.do",{yfpdm:yfpdm,yfphm:yfphm},function(json){
							if(json.errMessage){
		...
		jyxxb : function(){
			...
			//校验输入
			if(!/\d{10}/.test(yfpdm)||/^0+$/.test(yfpdm)){
				alert("销项正数发票代码填写有误！");
				return false;
		...
		addExtraInfo:function (){
			...
			_$("#zzspp_fsfpkj_form").append(extraDiv);
		...
		printfp:function (){
			...
			zzspp_fsfpkj.initPrintData();
			$.cookie("dyfs")=="1"?LODOP.PRINTA():LODOP.PRINT();
		},
		initYLData:function (){
			zzspp_fsfpkj.initPrintData(true);
		...
		initPage:function(){
			...
			ajaxLoad(ctxPath+"/zzspp_fs/check.do",{async:false},function(json){
