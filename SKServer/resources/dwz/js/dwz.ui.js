function initEnv() {
	...
	$.ajaxSetup({beforeSend:function(){
		if(window.pageToken&&$.cookie("pageToken")!=window.pageToken){
			alertMsg.error("您的浏览器已使用其他用户在新标签页中登录,此页面已为非法页面,请立即关闭此页面！",{okCall:function(){location.href=ctxPath}});
			return false;
		}
	}});
...
//					$form.find(':input.error').filter(':first').focus();
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			options.callback = eval($this.attr("callback") || "");
