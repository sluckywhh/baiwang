    ...
    ajaxDone: function (json) {
        if (json.statusCode == DWZ.statusCode.error) {
            if (json.message && alertMsg) alertMsg.error(json.message, {
                javaException: json.javaException
            });
...
(function ($) {
    ...
    $.fn.extend({
        ajaxUrl: function (op) {
            ...
            //op.type="POST";}
            $.ajax({
                type: op.type || op.data.type || 'POST',
                url: op.url,
                data: op.data,
                cache: false,
...
(function ($) {
    if ($.validator) {
        ...
        $.validator.addMethod("nsrsbh", function (value, element) {
            return this.optional(element) || /(^[a-zA-Z0-9]{9}$)|(^[a-zA-Z0-9]{15,20}$)/.test(value);
    ...
    $.ajaxSetup({
        beforeSend: function () {
            if (window.pageToken && $.cookie("pageToken") != window.pageToken) {
                alertMsg.error("您的浏览器已使用其他用户在新标签页中登录,此页面已为非法页面,请立即关闭此页面!", {
                    okCall: function () {
...
var alertMsg = {
    ...
    showJavaException: function () {
        ...
        if (div.length == 0) {
            div = $("<div class='javaException'>").appendTo(msg.find(".msg").css({
                "max-height": "400px"
            }));
...
function navTabAjaxDoneAndDialogAjaxDoneFather(json) {
    DWZ.ajaxDone(json);
    var arr = json.navTabId.split("|");
    if (json.statusCode == DWZ.statusCode.ok) {
        if (arr[1]) {
            var dialog = $("body").data(arr[1]);
    ...
    $.fn.extend({
        ...
        suggest: function () {
            ...
            return this.each(function () {
                ...
                $input.change(_show).click(false).keyup(function (event) {
    ...
    $.effect = {
        getDimensions: function (element, displayElement) {
            ...
            if (displayOrig == 'none') {
                ...
                if (isZero)
                    if (isZero) $(element).css("height", "0px");
            }
            return dimensions;
        }
    }
