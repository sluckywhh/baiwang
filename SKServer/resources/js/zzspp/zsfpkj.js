var zzspp_fpkj = function(){
	...
	return {
		...
		addZk:function (){
			...
			$.pdialog.open(ctxPath+"/zzspp/addZk.do","zzspp_addZk_dlg","添加折扣",{
				width:450,
				height:240,
				callback:function(){
					$("#zkhs").empty();
		...
		addExtraInfo:function (){
			...
			_$("#zzspp_fpkj_form").append(extraDiv);
		...
		cxgfxx:function (){
			var _this = this;
			$.pdialog.open(ctxPath+"/zzspp/gfxxDlg.do","zzspp_cxgfxx_dlg","查询购方信息",{
				width:600,
				height:215,
				callback:function(){
...
//									}			
		...
		jylsh:function (){
			var _this = this;
			$.pdialog.open(ctxPath + "/zzspp/jylshDlg.do?lshString=" + lshString,"zzspp_jylsh_dlg","请输入交易流水信息",{
				width:722,
				height:415,
				callback:function(){
		...
		judRepJylsh: function(lshString, gfsh, f, args){
			...
			ajaxLoad(ctxPath+"/zzspp/judRepJylsh.do", {async:false, lshString:lshString, gfsh:gfsh},function(json){
				if(json.size > 0){
					if(json.count > 0){
					...
					$.pdialog.open(ctxPath+"/zzspp/showRepJylsh.do?data="+json.data,"jylsh_show_dlg","重复交易流水列表",{
						width:935,
						height:600,
						callback:function(){
							$("#dlg_next_bt").click(function(){
		...
		fyxmdr:function(json){
			dialogAjaxDone(json);
			if(json.fyxm){
				zzspp_fpkj.setValueFromExcel(json.fyxm);
			}else{
				alert("没有找到符合条件的数据！");
				return;
			}
		...
		kjcg:function(json){
			...
			if (json.statusCode == DWZ.statusCode.ok){
				disableButtons(_$("#xz,#sc,#zk,#jg,#qd,#kj,#cxgfxx,#jylsh,#fyxm"));
		...
		printfp:function (){
			...
			zzspp_fpkj.initPrintData();
			$.cookie("dyfs")=="1"?LODOP.PRINTA():LODOP.PRINT();
		},
		initYLData:function (){
			zzspp_fpkj.initPrintData(true);
		...
		initPage:function(){
			...
			ajaxLoad(ctxPath+"/zzspp/check.do",{async:false},function(json){
				enableButtons(_$("#xz,#jg,#qd,#kj,#cxgfxx,#jylsh,#fyxm,#fyxmmb,#fyxmdrsm"));
