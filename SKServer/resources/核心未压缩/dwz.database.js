        ...
        bringBackSuggest: function(args){
            ...
            $box.find(":input").each(function(){
                ...
                for (var key in args) {
                    ...
                    if (name == inputName) {
                        $input.val(args[key]=="null"?"":args[key]);
                        break;
                    }
                }
            });
    ...
    $.fn.extend({
        ...
        suggest: function(){
            ...
            return this.each(function(){
                ...
                function _real_show(event){
                    if(event.type=="change"&&hasSel){
                        hasSel=false;
                        _close();
                        return;
                    }
                    //自定义项
                    if($input.attr("readonly")&&!$input.attr("ignoreReadonly")){
                        _close();
                        return;
                    }
                    var min = $input.attr("minL")||2;
                    if(min&&min>$.trim($input.val()).length){
                        _close();
                        return;
                    }
                    var offset = $input.offset();
                    var iTop = offset.top+$input.outerHeight();
                    ...
                    var postData = {};
                    postData["eventType"] = event.type;
                    postData["postValue"] = $input.val();
                    postData["postName"] = $input.attr("postField")||$input.attr("name");
                    $.ajax({
                        global:false,
                        ...
                        success: function(response){
                            ...
                            if ($lis.size() == 1 && event.keyCode != DWZ.keyCode.BACKSPACE&&$input.attr("selOne")=="true") {
                                _select($lis.eq(0));
                            } else if ($lis.size() == 0){
                                var jsonStr = "";
                                for (var i=0; i<suggestFields.length; i++){
                ...
                $input/*.focus(_show)*/.change(_show).click(false).keyup(function(event){
