...
var alertMsg = {
	...
	_open: function(type, msg, buttons){
		...
		for (var i = 0; i < buttons.length; i++) {
			...
			if(buttons[i].call==this.showJavaException){
				jButs.eq(i).removeAttr("onclick");
			}
		}
	},
	close: function(){
	...
	_alert: function(type, msg, options) {
		...
		if(options&&options.javaException){
			buttons.push({name:"详细信息", call:this.showJavaException});
			var javaEx = $("#javaException");
			if(javaEx.length==0){
				javaEx = $("<div id='javaException'>").appendTo($("body")).hide();
			}
			javaEx.text(options.javaException);
		}
	...
	showJavaException: function(){
		var msg = $("#alertMsgBox");
		var div = $("#alertMsgBox").find(".javaException");
		if(div.length==0){
			div = $("<div class='javaException'>").appendTo(msg.find(".msg").css({"max-height":"400px"}));
			div.before("<h1 style='color:red'>请将以下详细信息复制并发送给管理人员</h1>")
			div.html($("#javaException").text());
			msg.css({"left":"30%","width":"60%"});
		}
	}
};
